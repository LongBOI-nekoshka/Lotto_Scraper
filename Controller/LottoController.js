...
class LottoController {
    static scrape (gameMode) {
        var lotto_array = [];
        var lotto_high = [];
        var occurence = {};
        var data = new FormData();
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlStartMonth', 'November');
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlStartDate', '20');
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlStartYear', '2011');
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlEndMonth', 'November');
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlEndDay', '20');
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlEndYear', '2021');
        data.append('ctl00$ctl00$cphContainer$cpContent$ddlSelectGame', gameMode);
        ...
        return axios(config).then(function (response) {
            ...
            cherry('#cphContainer_cpContent_GridView1 > tbody > tr').each((index, element) => {
                if(cherry(cherry(element).find("td")[1]).text() !== "") {
                    ...
                    cherry(cherry(element).find("td")[1]).text().split('-').forEach(function(n) {
                        lotto_high.push(n);
                    })
                    
                    for (const num of lotto_high) {
                        occurence[num] = occurence[num] ? occurence[num] + 1 : 1;
                    }
                }
            });
            var sum = 0;

            Object.values(occurence).map((element) => {
                sum += element;
            });

            return Object.entries(occurence).sort((a,b) => b[1]-a[1]).map(element=>{
                var test = {
                    label:element[0],
                    occurence:element[1],
